...
class mock_HardwareI2c : public II2c {
 public:
  /**
   ...
   */
  static std::optional<std::shared_ptr<mock_HardwareI2c>> create(core::ILogger &logger,
                                                            const std::uint8_t bus);
  mock_HardwareI2c(core::ILogger &logger, const int file_descriptor);
  ~mock_HardwareI2c();
