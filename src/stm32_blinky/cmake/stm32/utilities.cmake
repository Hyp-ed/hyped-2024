...
# A CMSIS or HAL driver can specify 'cube' as version number to indicate that
# the driver is taken from the Cube repository
set(STM32_FETCH_FAMILIES
    C0
    F0
    F1
    F2
...
foreach(FAMILY ${STM32_FETCH_FAMILIES})
  ...
  list(
    GET
    STM32_FETCH_CUBE_VERSIONS
    ${IDX}
    CUBE_VERSION
  )
  list(
    GET
    STM32_FETCH_CMSIS_VERSIONS
    ${IDX}
    CMSIS_VERSION
  )
  list(
    GET
    STM32_FETCH_HAL_VERSIONS
    ${IDX}
    HAL_VERSION
  )
  ...
  if(CMSIS_VERSION STREQUAL cube)
    ...
    FetchContent_Declare(
      STM32-CMSIS-${FAMILY}
      GIT_REPOSITORY
        https://github.com/STMicroelectronics/cmsis_device_${FAMILY_L}/
  ...
  if(HAL_VERSION STREQUAL cube)
    ...
    FetchContent_Declare(
      STM32-HAL-${FAMILY}
      GIT_REPOSITORY
        https://github.com/STMicroelectronics/stm32${FAMILY_L}xx_hal_driver/
      GIT_TAG ${HAL_VERSION}
      GIT_PROGRESS TRUE
    )
  endif()
...
function(stm32_fetch_cube)
  foreach(FAMILY ${ARGV})
    ...
    if(STM32_CUBE_${FAMILY}_PATH)
      message(
        VERBOSE
        "STM32_CUBE_${FAMILY}_PATH specified, skipping fetch for ${CUBE_NAME}"
      )
      continue()
    endif()
...
function(stm32_fetch_cmsis)
  ...
  foreach(FAMILY ${ARGV})
    if(STM32_USE_CMSIS_FROM_CUBE_${FAMILY})
      ...
      message(
        STATUS "Cube fetched for ${FAMILY} at ${STM32_CUBE_${FAMILY}_PATH}"
      )
