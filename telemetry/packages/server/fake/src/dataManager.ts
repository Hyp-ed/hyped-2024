...
export class SensorMap {
    ...
    private sensors: SensorInstance<typeof sensors.default[keyof typeof sensors.default]>[] = [];
     ...
     */
    constructor(private sensorsToRun: string[]) { // optional until specific sensor choice logic is added
        ...
        const simulationInterval = setInterval(() => {
            ...
            this.sensors.forEach( sensor => {
                if (this.globalTime % sensor.sampling_time == 0) {
                    ...
                    const readings: Readings = !random ? sensor.getData(this.globalTime / 1000)
            ...
            if (this.globalTime >= runTime) { clearInterval(simulationInterval); }
        }, interval);
    }
    ...
    private instantiateSensors(): void {
        ...
        for (const sensorType in sensorData) {
            this.sensors.push( new sensors.default[sensorType]( sensorData[sensorType]) );
    ...
    private resetSampledState(): void {
        Object.keys(this.isSampled).forEach( sensor => this.isSampled[sensor] = false );
    }
    ...
    private getDependecies(sensorType: string): Record<string, any> {
        ...
        Then when the pressure gauges are called upon to cough up some data for the telemetry team, the code would
          ...
          pressure.
        */
       return { /* dependencies categorised by sensor */ };
    }
...
/* old data gen logic
   ...
   generate(): void {
    ...
    setTimeout(() => {
        ...
        this.sensors.forEach( (sensor, i) => {
            if (this.globalTime < Sensor.nextSamplingTimes[sensor.type]) { return; }
...
generateRandom(): void {
    ...
    setTimeout( () => {
        ...
        for (const sensor in this.sensorData) {
            ...
            if (this.globalTime < Sensor.nextSamplingTimes[sensor]) { continue; }
