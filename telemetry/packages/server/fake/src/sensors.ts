import { Readings, LiveReading, Limits, Utilities } from '../index';
...
export abstract class Sensor {
  ...
  constructor({
    ...
    Object.assign(this, { type, format, limits, rms_noise, sampling_time });
    this.time = 0;
...
class Motion extends Sensor {
  ...
  getData(t: number): Readings {
    ...
    return {
      ...
      (Sensor.globalReadings.motion.displacement +=
    ...
    for (const unit in this.readings) {
      switch (unit.replace(/_[^_]*\d$/, '')) {
        ...
        case 'velocity':
          this.readings.velocity +=
            this.readings.acceleration * this.sampling_time;
          break;
        case 'displacement':
          this.readings.displacement +=
            this.readings.velocity * this.sampling_time;
          break;
        default:
          break;
      }
    }
    this.update(this.readings);
