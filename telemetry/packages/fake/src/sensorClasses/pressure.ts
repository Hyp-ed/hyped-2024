...
export class Pressure extends Temperature {
  ...
  constructor(data: LiveReading) {
    ...
    this.pResBrakes0 = data['pressure_brakes_reservoir'];
    this.pResSusp0 = data['pressure_active_suspension_reservoir'];
  }
  ...
  getData(): Readings {
    ...
    newData['pressure_active_suspension_reservoir'] = this.idealGasLaw('suspension')
    ...
    return Object.fromEntries(
      Object.entries(newData).map(([key, value]) => {
        return [
          key,
          utils.round2DP((
            value + utils.gaussianRandom(this.rms_noise))
            * 10**(-5) // convert back to bar
          )
        ];
      })
    )
  ...
  private idealGasLaw(loc: 'brakes' | 'suspension'): number {
    this.temp += this.acceleration < 0 && loc == 'brakes'
  ...
  private brakePressure(): number {
    ...
    if (this.acceleration > 0 && p > atm) {
      ...
      this.temp += Math.abs(this.acceleration) * 5 * this.coefficients.brakingFactor;
